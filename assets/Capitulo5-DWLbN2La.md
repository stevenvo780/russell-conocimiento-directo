---
title: "La arquitectura de lo ausente"
subtitle: "Exposición oral del capítulo 5 de Los problemas de la filosofía"
author: "Steven Vallejo"
source_author: "Bertrand Russell"
source_edition: "Editorial Labor, 1995"
chapter: 5
language: es
estimated_duration: "27 minutos de exposición; 25–30 con interacción"
slides: 14
---
...
flowchart TD
    ...
    C --> R["POR REFERENCIA<br/>mediante «el F»"]
    R -->|implica| V
    V -->|se funda finalmente en| D
    R -->|se funda finalmente en| D

    classDef raiz fill:#24262d,color:#fff,stroke:#888,stroke-width:2px;
    classDef directo fill:#d69a52,color:#17130f,stroke:#f2c57c,stroke-width:2px;
    classDef referencia fill:#396a8b,color:#fff,stroke:#79b6d8,stroke-width:2px;
    class K,C,V raiz;
    class D directo;
    class R referencia;
```
...
```mermaid
flowchart LR
    A["VER Y TOCAR"] --> B["color · forma<br/>dureza · suavidad"]
    B --> C["DATOS SENSORIALES<br/>directamente conocidos"]
    C --> D["VERDAD PUENTE<br/>un objeto físico<br/>causa estos datos"]
    D --> E["DESCRIPCIÓN<br/>«el objeto que los causa»"]
    E --> F["MESA FÍSICA<br/>conocida por referencia"]

    classDef directo fill:#d69a52,color:#17130f,stroke:#f2c57c,stroke-width:2px;
    classDef puente fill:#24262d,color:#fff,stroke:#888,stroke-width:2px;
    classDef referencia fill:#396a8b,color:#fff,stroke:#79b6d8,stroke-width:2px;
    class A,B,C directo;
    class D puente;
    class E,F referencia;
```
...
flowchart TB
    ...
    D --> I["INTROSPECCIÓN<br/>vida mental propia"]
    ...
    classDef directo fill:#d69a52,color:#17130f,stroke:#f2c57c,stroke-width:2px;
    classDef incierto fill:#66513a,color:#fff7e8,stroke:#f2c57c,stroke-dasharray:5 4;
    classDef referencia fill:#396a8b,color:#fff,stroke:#79b6d8,stroke-width:2px;
    class D,S,M,I,U directo;
    class Y incierto;
    class N,F,O,R referencia;
```
...
```mermaid
flowchart LR
    Y["¿YO?<br/>sujeto probable"] -->|conoce| A["ACTO<br/>ver el sol"]
...
```mermaid
flowchart TD
    F["DESCRIPCIÓN DEFINIDA<br/>«el F»"] --> E{"¿EXISTE AL MENOS<br/>UN OBJETO F?"}
    E -->|no| N["NO HAY REFERENTE"]
    E -->|sí| U{"¿HAY COMO MÁXIMO<br/>UNO?"}
    U -->|no| A["NO IDENTIFICA<br/>UN OBJETO ÚNICO"]
    U -->|sí| K{"¿SÉ QUE UN OBJETO<br/>DIRECTAMENTE CONOCIDO<br/>ES EL F?"}
    K -->|sí| I["IDENTIFICACIÓN"]
    K -->|no| R["MERAMENTE CONOCIDO<br/>POR REFERENCIA"]

    classDef pregunta fill:#24262d,color:#fff,stroke:#888,stroke-width:2px;
    classDef fallo fill:#472f37,color:#fff,stroke:#b66b76,stroke-width:2px;
    classDef salida fill:#396a8b,color:#fff,stroke:#79b6d8,stroke-width:2px;
    class F,E,U,K pregunta;
    class N,A fallo;
    class I,R salida;
```
...
```mermaid
flowchart LR
    B0["BISMARCK<br/>respecto de sí"] -->|más distancia| B1["QUIEN LO TRATÓ<br/>datos directos → Bismarck referido"]
    B1 -->|más distancia| B2["HISTORIADOR<br/>testimonio"]
    B2 -->|más distancia| B3["HOMBRE DE LA<br/>MÁSCARA DE HIERRO"]
    B3 -->|más distancia| B4["HOMBRE QUE VIVIÓ<br/>MÁS TIEMPO"]

    classDef cerca fill:#d69a52,color:#17130f,stroke:#f2c57c,stroke-width:2px;
    classDef medio fill:#66513a,color:#fff7e8,stroke:#d69a52,stroke-width:2px;
    classDef lejos fill:#396a8b,color:#fff,stroke:#79b6d8,stroke-width:2px;
    class B0 cerca;
    class B1,B2 medio;
    class B3,B4 lejos;
```
...
```mermaid
flowchart LR
    P["PARTICULARES<br/>sonidos · datos visuales · recuerdos"] --> C["CONSTITUYENTES<br/>directamente conocidos"]
    U["UNIVERSALES<br/>hombre · asesinar · relación"] --> C
    C --> D["DESCRIPCIÓN<br/>comprensible"]
    D --> J["JULIO CÉSAR<br/>no experimentado"]
    J --> M["CONOCIMIENTO MÁS ALLÁ<br/>DE LA EXPERIENCIA PRIVADA"]

    classDef directo fill:#d69a52,color:#17130f,stroke:#f2c57c,stroke-width:2px;
    classDef referencia fill:#396a8b,color:#fff,stroke:#79b6d8,stroke-width:2px;
    class P,U,C directo;
    class D,J,M referencia;
```
...
```mermaid
flowchart LR
    D["DIRECTO<br/>ANCLAJE"] -->|color · forma · resistencia| E["DATOS DE ESTA MESA"]
    D --> U["UNIVERSALES<br/>APREHENDIDOS"]
    E --> V["VERDAD<br/>PUENTE"]
    U --> V
    V -->|«el objeto físico<br/>que los causa»| R["REFERENCIA<br/>ALCANCE"]
    R --> M["UN MUNDO MAYOR<br/>QUE LA EXPERIENCIA PRIVADA"]
